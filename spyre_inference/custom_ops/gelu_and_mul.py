...
`forward_native` slices the fused `[..., 2*d]` tensor on the last dim; that
indirect-access slice used to corrupt on Spyre in eager mode, but torch-spyre now
handles it directly, so `forward_oot` runs `forward_native` as-is. Mirrors
`SpyreSiluAndMul` with GELU instead of SiLU.
"""
